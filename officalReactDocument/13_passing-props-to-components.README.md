...
- React 컴포넌트는 props를 이용해 서로 통신함
- 모든 부모 컴포넌트는 props를 줌으로써 몇몇의 정보를 자식 컴포넌트에게 전달할 수 있음
- HTML 속성처럼 보일 수 있지만, 객체, 배열, 함수 등 모든 JavaScript 값을 전달할 수 있음
...
- `<img>` 태그에 전달할 수 있는 props는 미리 정의되어 있음
- 자신이 생성한 `<Avatar>`와 같은 어떤 컴포넌트든 props를 전달할 수 있음
